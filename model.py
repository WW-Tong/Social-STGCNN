...
class Decoder(nn.Module):
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple,obs_traj,obs_traj_rel):
        npeds = last_pos.size(1)           # 人数  N V C
        pred_traj_fake_rel = []             
...
class EndpointGenerate(nn.Module):
    ...
    def forward(self,obs_traj,obs_traj_rel):
        ...
        obs_traj_embedding = self.spatial_embedding(obs_traj_rel.view(-1, 2))   # 展成n*2输入线性嵌入层,输出16 TNV*16
        ...
        last_pos = obs_traj[:, :, :, -1]    # 最后一刻  N V C
        obs_speed = torch.sum(obs_traj_rel, dim=3)/8      #  N V C
        move = obs_speed*12     
        npeds = last_pos.size(1)        # 行人编号
